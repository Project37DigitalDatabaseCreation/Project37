        ...
        <div class="form-group row">
            ...
            <div class="col-md-6">
                <select id="organization" class="form-control" name="organization"
                    required v-model="selected_org" v-on:change="populateProjects">
                    <option v-for="organization in organizations" :value="organization.id"
                        :key="organization.id">{{ organization.data().title }}</option>
                </select>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-md-6">
                <select id="project" class="form-control" name="project"
                    :disabled="projectSelectEnabled == 0" v-on:change="populateReviews"
                    required v-model="selected_proj">
                    <option v-for="project in projects" :value="project.id"
                        :key="project.id">{{ project.data().title }}</option>
                </select>
            </div>
        </div>
        <div>
            <button class="btn btn-primary" id="show-modal"
                :disabled="projectNewReviewEnabled == 0" @click="populatePopOut()">
            ...
            <tbody>
                <tr v-for="review in reviews" :key="review.id">
                    ...
                    <td><button class="btn btn-primary"
                            @click="populatePopOut(review)">Edit</button></td>
                    <td><button class="btn btn-primary"
                            @click="deleteReview(review)">Delete</button></td>
                </tr>
            </tbody>
        </table>
        <!-- use the modal component, pass in the prop -->
        <modal v-if="showModal" v-on:edit-review="submitEdit"
            :selected_review="this.selected_review" :reviewers="this.reviewers"
