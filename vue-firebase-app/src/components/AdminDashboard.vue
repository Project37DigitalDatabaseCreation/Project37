...
<template>
    <div class="container scrollcontainer">
        ...
        <div class="row justify-content-center" style="font-family: Calibri;">
            <div class="col-sm-2">
                <div class="card">
                    <div class="card-body"
        ...
        <table class="table mt-5">
            ...
            <tbody>
                <tr v-for="review in inProgressReviews" :key="review.id">
                    ...
                    <td><button class="btn save"
                            @click="openReview(review.id)">Open</button></td>
                </tr>
            </tbody>
        </table>
        <div v-if="in_progress_reviews.length > PAGE_SIZE">
            <button @click="prevInProgressPage"
                :disabled="in_progress_page === 1">Previous</button>
            <button @click="nextInProgressPage"
                :disabled="in_progress_reviews.length <= in_progress_page * PAGE_SIZE">Next</button>
        </div>
        ...
        <div v-if="completed_reviews.length > PAGE_SIZE">
            <button @click="prevCompletedPage"
                :disabled="completed_page === 1">Previous</button>
            <button @click="nextCompletedPage"
                :disabled="completed_reviews.length <= completed_page * PAGE_SIZE">Next</button>
        </div>
    </div>
</template>
<script>
...
export default {
    ...
    methods: {
        ...
        nextInProgressPage() {
            if (this.in_progress_page * this.PAGE_SIZE < this.in_progress_reviews.length)
                this.in_progress_page++
        },
        prevCompletedPage() {
            if (this.completed_page > 1) this.completed_page--
        },
        nextCompletedPage() {
            if (this.completed_page * this.PAGE_SIZE < this.completed_reviews.length)
                this.completed_page++
        },
        openReview(reviewId) {
                    ...
                    .then((result) => {
                        result.forEach((doc) => {
                            ...
                            if (score.standard_ref) {
                                const scoreStandard = this.standards.find(function (
                                    standard
                                ) {
